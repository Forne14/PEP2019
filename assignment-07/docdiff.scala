...
def prod(lst1: List[String], lst2: List[String]) : Int = {
  val firstListMap = occurrences(lst1)
  val secondListMap = occurrences(lst2)
  val dotProductSet = {
    firstListMap.keySet.intersect(secondListMap.keySet).map(
      k=>k->(firstListMap(k),secondListMap(k))
    )
  }

  val listOfDotProdValues= {
    for((sk,(k,v))<-dotProductSet.toList)yield {
      k*v
    }
  }

  val total = listOfDotProdValues.sum
  total
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
    val dotProductTop = prod(lst1,lst2).toDouble
    val dotProductBottom = {
    if(prod(lst1,lst1).toDouble>=prod(lst2,lst2).toDouble){
      prod(lst1,lst1).toDouble
    }else{
      prod(lst2,lst2).toDouble
    }
  }
    val result = dotProductTop/dotProductBottom
    result
}

def similarity(s1: String, s2: String) : Double = {
  val subject1 = clean(s1)
  val subject2 = clean(s2)
  val similarityOfTwo = {
    overlap(subject1,subject2)
  }
  similarityOfTwo
}
...
/* Test cases
Double = 0.8490566037735849
res3: Double = 0.8490566037735849
