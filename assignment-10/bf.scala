...
def load_bff(name: String): String ={
  val x = Try(Source.fromFile(name).getLines).getOrElse(Nil).toList.mkString
  x
}
...
//prog = program
//pc = program counter
//level = checks brackets are on the dame level
...
def jumpRight(prog: String, pc: Int, level: Int) : Int =  {
  ...
  else if(prog.charAt(pc)=='['){
    jumpRight(prog, pc+1, level+1);
  }
  else if(prog.charAt(pc)==']' && level>0){
